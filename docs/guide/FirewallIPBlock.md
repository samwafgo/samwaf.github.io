...
页面顶部分为 **手工封禁** 与 **订阅来源** 两个页签。**手工封禁** 页提供 5 个统计卡片，实时展示当前封禁规则概况：总数、已生效、已失效、已过期，以及 **订阅落地(系统层)**（威胁情报订阅落地到系统防火墙的生效条数）。点击 **订阅落地(系统层)** 卡片可直接切换到 **订阅来源** 页查看逐渠道详情。
...
- 点击 **清理过期**，会弹出确认框；确认后 **撤销防火墙规则并删除所有已过期的封禁记录（不可恢复）**。永久封禁及未过期的记录不受影响。
...
切换到顶部 **订阅来源** 页签，可只读查看落地到系统防火墙的 [威胁情报IP订阅](./ThreatIP.md) 各渠道汇总：名称、渠道短码、落地层、启用状态、**已生效落地数**、上次状态、上次同步时间。

出于性能考虑，订阅 IP 不会逐条写入本页的封禁列表，而是按渠道汇总展示。点击某渠道的 **查看IP** 可分页只读浏览该渠道的具体 IP/网段，并支持按 IP/网段片段过滤。

::: tip 已生效落地数
停用的渠道其落地数显示为 0（已从系统防火墙移除），后面会以灰字提示快照条数（再次启用即可秒回灌），因此该列反映的是“当前实际生效”的条数，而非订阅收录总量。
:::
...
- **过期记录如何清理？** 点击顶部的 **清理过期** 按钮，确认后会撤销防火墙规则并删除所有已过期记录（不可恢复）；永久封禁与未过期记录不受影响。
