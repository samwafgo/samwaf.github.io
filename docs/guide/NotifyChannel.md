...
对方平台接收消息的地址。仅支持 `http` / `https`，且目标默认必须是**公网地址**——填内网地址（如 `127.0.0.1`、`192.168.x.x`）保存时会被拒绝。

::: tip 告警平台建在内网？
自建在内网的 ntfy、Gotify 等平台，可由服务器管理员在 `conf/config.yml` 的 `security.outbound_allowed_hosts` 里按主机名、IP 或网段声明放行（逗号分隔，如 `192.168.10.0/24,notify.intranet.lan`），保存并重启 SamWaf 后生效。该文件不走管理端界面和 API，只能直接在服务器上修改。
:::
...
Webhook 地址仅支持 `http` / `https`，且目标默认必须是**公网地址**。指向 `127.0.0.1`、`192.168.x.x`、`10.x.x.x` 等内网地址，或跳转到内网地址的链接，都会被拒绝。确需推送到内网告警平台时，在 `conf/config.yml` 的 `security.outbound_allowed_hosts` 声明该主机/IP/网段并重启即可。
