...
| 规则名 | 规则的名称 |
| 规则编号 | 规则的唯一编号（系统生成） |
| 优先级 | 规则的 salience 优先级数值，数值越大越优先（详见 6.1 与第 7 节） |
| 规则版本 | 规则的版本号 |
...
| 规则名称 | 规则的可读名称 |
| 防护网站 | 选择规则生效的受保护站点 |
| 规则优先级 | salience 优先级数值，数值越大越优先，范围 0~10000，默认 10。**站点规则与全局规则会放在一起按该数值仲裁**，详见第 7 节 |
| 防护编排方式 | 选择 `界面编排` 或 `手工代码编排` |
| 规则状态 | 规则开关，`已生效` / `已关闭` |

::: warning
**手工代码编排**模式下，脚本里手写的 `salience` 会被「规则优先级」这一项的数值覆盖，最终以表单里填的为准。要改优先级请改这个输入框，改脚本里的数字不生效。
:::
...
- 模块名**不区分大小写**，`RF.Allow("cc")` 与 `RF.Allow("CC")` 等效。
- 可一次跳过多个模块，用逗号分隔：`RF.Allow("SQLI","XSS","CC");`
- 不带参数的 `RF.Allow()` 只表示「不被自定义规则拦截」，**不跳过**任何内置检测。
- 填写白名单以外的模块名会在保存/校验规则时报错。
...
规则可以配在**某个具体网站**下，也可以配在**全局网站**下（对所有站点生效）。当一次请求同时命中了站点规则和全局规则，两边会**放在一起按 salience 仲裁**：

| 情况 | 生效结果 |
| --- | --- |
| 只有一边命中 | 用命中的那一边 |
| 两边都命中，salience 不同 | **数值大的一方胜出**（不分站点还是全局） |
| 两边都命中，salience 相同 | 按 **拦截 > 放行 > 仅记录** 取。即同优先级时，全局的拦截会盖住站点的放行 |

所以，**站点白名单要压过全局拦截规则，必须把站点规则的优先级数值调得比全局规则更大**。

示例：全局规则屏蔽所有国外 IP（优先级 10），但希望某个文件共享站点对一批可信 IP 放行——站点放行规则的优先级要填成大于 10 的值（例如 100）：

```
// 全局网站下的规则：屏蔽国外访问，优先级 10
rule Rglobal_block_oversea "屏蔽国外访问" salience 10 {
    when
        MF.COUNTRY != "中国"
    then
        RF.Deny();
...
```
// 文件共享站点下的规则：可信 IP 放行，优先级 100（大于 10，才能压过全局拦截）
rule Rsite_allow_partner "合作方IP放行" salience 100 {
    when
...
::: warning
如果把上面站点规则的优先级也写成 10，则与全局规则同级，按「拦截 > 放行」取，请求仍会被全局规则拦截。这也是老版本的固定行为——老配置里两边通常都是默认的 10，升级后行为不变。
:::

::: tip
`RF.Allow()` 与 `RF.AllowAll()` 在优先级仲裁上**完全一样**，区别只在于放行之后跳过多大范围的后续检测（见 7.1）。不要用 `RF.AllowAll()` 来「提高优先级」，要提高优先级请调 salience 数值。
:::
